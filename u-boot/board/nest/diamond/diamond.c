 ...
 *
 *  Description:
 *    This routine copies the Diamond serial string (u-boot 'serial#')
 *    as an extension to the ATAG_SERIAL parameter passed from u-boot
 *    to the kernel.
 *
...
void get_board_serial(struct tag_serialnr *serialnr)
{
	...
	if (serial) {
		strncpy(serialnr->diamond_serial, serial, sizeof(serialnr->diamond_serial));
	} else {
		serialnr->diamond_serial[0] = '\0';
	}

	serialnr->high = high;
	serialnr->low  = low;
#if defined(NEST_BUILD_CONFIG_DIAGNOSTICS)
	strncpy(serialnr->diamond_serial, "0000000000000000", 16);
